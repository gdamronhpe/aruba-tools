...
def load_saved_requests() -> list[dict]:
    ...
    if not path.exists():
        try:
            path.write_text(json.dumps(DEFAULT_SAVED_REQUESTS, indent=2), encoding="utf-8")
        except Exception as e:
...
def launch_ui():
    ...
    if getattr(sys, 'frozen', False):
        # The _MEIPASS attribute is set by PyInstaller in a one-file bundle
        # This is the path to the temporary directory where data files are extracted
        bundle_dir = sys._MEIPASS
    else:
        # Not frozen (running as a script), use the current directory
        bundle_dir = os.path.dirname(os.path.abspath(__file__))

    icon_path = Path(bundle_dir) / "MultiSwitchCXplorer.ico"
    if icon_path.exists():
    ...
    tab_control.pack(side=tk.RIGHT, fill=tk.BOTH, expand=True, padx=10, pady=10)
    ...
    saved_requests_frame.pack(fill=tk.X, padx=10, pady=5)
    saved_requests = load_saved_requests()
    saved_combo = ttk.Combobox(saved_requests_frame, state="readonly")
    saved_combo.pack(fill=tk.X, padx=10, pady=5)

    def refresh_saved_combo():
        saved_combo["values"] = [r.get("name", "Unnamed") for r in saved_requests]
        if saved_requests:
    ...
    def on_saved_selected(_event=None):
        apply_saved_request(saved_combo.current())
    ...
    saved_combo.bind("<<ComboboxSelected>>", on_saved_selected)
    btns_saved = ttk.Frame(saved_requests_frame)
    btns_saved.pack(fill=tk.X, padx=10, pady=(0, 6))
    ttk.Button(btns_saved, text="Delete Saved Request", command=delete_saved_request).pack(side=tk.LEFT)
    refresh_saved_combo()
    ...
    # Generate CLI button
    ttk.Button(controls_frame, text="Generate CLI...", command=lambda: open_cli_builder_popup(
        root,
        username_entry, password_entry, switches_entry,
        endpoint_combobox, version_value, depth_entry,
        selector_value, ssl_var, concurrent_spin, attributes_entry
    ...
    def start_api_calls(append=False, switches_override=None):
        ...
        run_button_ref.config(text="Stop API Requests", bg="red", command=toggle_run_stop) # Change button to Stop
    ...
    def stop_api_calls():
        ...
        run_button_ref.config(text="Stopping...", bg="gray", state="disabled")
    ...
    def run_api_calls_threaded(switches, username, password, endpoint, version, depth, selector, verify_ssl,
        ...
        def _finish_ui():
            run_button_ref.config(text="Run API Requests", bg="#4CAF50", state="normal", command=toggle_run_stop)
    ...
    help_btn = tk.Button(main_frame, text="Help", command=lambda: show_help_guide(APP_VERSION), font=("Arial", 8, "bold"), relief="flat", bg="lightgray") # Pass APP_VERSION
    help_btn.place(relx=1.0, x=-5, y=5, anchor="ne")
